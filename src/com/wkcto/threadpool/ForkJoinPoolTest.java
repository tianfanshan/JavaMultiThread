...
public class ForkJoinPoolTest {
    //激素那数列的和，需要返回结果，可以定义任务继承RecursiveTask
    private static class CountTask extends RecursiveTask<Long>{

        private static final int THRESHOLD = 10000;//定义数据规模的阈值，允许计算10000个数内的和,超过改阈值的数列就需要分解
        ...
        protected Long compute() {
            ...
            if (end - start < THRESHOLD){
                ...
                //start = 0, end = 200000, step = 2000, 如果计算[0,200000]范围内的数列的和，把该范围的数列分解为100个小人物，每个人物计算2000个数即可
                //注意，如果任务划分的层次很深，即THRESHOLD阈值太小，每个任务的计算量很小，层次划分就会很深，可能出现两种情况：
                    //一是系统内的线程数量会越积越多，导致性能下降严重；二是分解次数过多，方法调用过多可能会导致栈溢出
                //创建存储任务的集合
